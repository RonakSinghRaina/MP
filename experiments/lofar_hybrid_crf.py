"""
HybridCRFNet on the REAL LOFAR dataset -- the Chen et al. 2023 CRF experiment.

IDENTICAL protocol to experiments/lofar_hybrid.py (same split, same leakage
guard, same budget, same metrics) so the two are directly comparable. The ONLY
difference is the model: HybridRFINet with a differentiable CRF head bolted on,
11 extra parameters.

READ src/hybrid_rfi_package/hybrid_crf_model.py FIRST. It records that a
frozen post-hoc CRF was already measured and buys +0.0012 at best, inside the
seed noise, and that Chen et al.'s own parameters are destructive here
(0.6592 -> 0.4884). This script exists to test the one case that bolting on
cannot: END-TO-END training, where the backbone adapts to the CRF.

THREE MODES
-----------
  --crf_mode frozen    load a trained backbone, FREEZE it, train only the 11
                       CRF parameters. Cleanest isolation -- any change is
                       100% attributable to the CRF. Fastest. Needs --init_from.
  --crf_mode finetune  load a trained backbone, train everything at a lower LR.
  --crf_mode scratch   train the whole thing from random init at the full
                       28,000-step budget, matching lofar_hybrid.py exactly.

COST (measured, RTX 3060, 512x512, batch 1)
  plain hybrid base 8        37 ms/step
  + CRF k=7, 5 iters        379 ms/step   -> 2.9 h for 28,000 steps
  + CRF k=5, 5 iters        280 ms/step
So `scratch` is a ~3 h run per seed; `frozen` needs far fewer steps because
there are only 11 parameters to fit.

The original HybridRFINet docstring follows.
---------------------------------------------------------------------------
...
from hybrid_crf_model import HybridCRFNet      # noqa: E402
...
def main():
    ...
    ap.add_argument("--crf_kernel", type=int, default=7, help="odd; 5 is ~1.4x faster")
    ap.add_argument("--crf_iters", type=int, default=5, help="5 = Chen et al.")
    ap.add_argument("--crf_mode", choices=["frozen", "finetune", "scratch"], default="frozen")
    ap.add_argument("--init_from", default=None, help="path to a trained HybridRFINet best.pt")
    ap.add_argument("--no_sigma_prior", action="store_true",
                    help="drop Chen et al.'s adaptive 2-sigma unary prior")
    ap.add_argument("--chen_init", action="store_true",
                    help="use the paper's lambda_a=3, lambda_s=20 (measured destructive here)")
    ap.add_argument("--output_dir", default=None)
    a = ap.parse_args()

    out = a.output_dir or os.path.join(_ROOT, "runs", "lofar",
                                       "hybridcrf_b{:02d}_{}_{}_seed{}".format(
                                           a.base, a.norm, a.crf_mode, a.seed))
    ...
    model = HybridCRFNet(1, 2, base=a.base, depth=a.depth, dropout=a.dropout,
                         crf_kernel=a.crf_kernel, crf_iters=a.crf_iters,
                         use_sigma_prior=not a.no_sigma_prior,
                         chen_init=a.chen_init).to(device)
    if a.init_from:
        sd = torch.load(a.init_from, map_location=device)
        model.load_backbone(sd)
        print("  warm-started backbone from {}".format(a.init_from))
    elif a.crf_mode == "frozen":
        raise SystemExit("\n--crf_mode frozen needs --init_from <best.pt>: there is "
                         "nothing to freeze otherwise.\n")
    if a.crf_mode == "frozen":
    ...
    print("  parameters        : {:,} total, {:,} trainable  ({})".format(
        n_par, n_train_par, a.crf_mode))
    print("  CRF head          : {} parameters, k={} x{} iters{}".format(
        model.crf_parameter_count(), a.crf_kernel, a.crf_iters,
        ", Chen et al. init" if a.chen_init else ""))
    ...
    opt = torch.optim.Adam([p for p in model.parameters() if p.requires_grad], lr=a.learning_rate)
    ...
    while prog["epochs_completed"] < a.total_epochs:
        ...
        model.train()
        if a.crf_mode == "frozen":
            model.backbone.eval()          # keep dropout/norm frozen too
        for _ in range(n_ep):
            tot, nb = 0.0, 0
    ...
    print("\n" + "=" * 74)
    print("  RESULT — HybridCRFNet on real LOFAR, human ground truth")
    print("=" * 74)
